...
class TestOsm2youSettings:
    ...
    def test_logging_settings(self):
        ...
        assert file_handler.baseFilename, (
    ...
    def test_csrf_settings(self):
        ...
        assert len(settings.CSRF_TRUSTED_ORIGINS), (
    ...
    def test_cors_settings(self):
        ...
        assert "access-control-allow-origin" in settings.CORS_ALLOW_HEADERS, (
            "Check access-control-allow-origin is present in CORS_ALLOW_HEADERS"
        )

        assert len(settings.CORS_ORIGIN_WHITELIST), (
    ...
    def test_static_media_settings(self):
        assert hasattr(settings, "STATIC_URL") and len(settings.STATIC_URL), (
            """Check that STATIC_URL settings filled."""
        )

        assert hasattr(settings, "MEDIA_URL") and len(settings.MEDIA_URL), (
            """Check that MEDIA_URL settings filled."""
        )

    def test_swagger_settings(self):
        assert hasattr(settings, "SWAGGER_SETTINGS"), (
            """Check that STATIC_URL settings filled."""
        )

        assert "drf_yasg" in settings.INSTALLED_APPS, (
